...
public class VSPlayerScript : MonoBehaviour
{
    ...
    public Text InputText;       //PlayerInputの表示
    // --- ゲームで使用する変数 ---
    private int playerGuessCount = 0;
    private int cpuGuessCount = 0;
    private const int maxcunt = 6;
    ...
    public string gameMode = "Setup_p1"; // "Setup", "PlayerTurn", "CPUTurn", "GameOver"
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Return) || Input.GetKeyDown(KeyCode.KeypadEnter))
        {
            ...
            else if (gameMode == "Setup_p2")
            {
                HandleOppositPlayerBaseInput();
            }
            else if (gameMode == "PlayerTurn")
            {
                HandlePlayerGuess();
            }
            else if (gameMode == "OppositPlayerTurn")
            {
                HandleOppositPlayerGuess();
            }
        ...
        {
            // 最初の指示メッセージ
            InputText.text = $"Player1の秘密の3桁の数字（重複なし）を入力し、Enterで決定してください。\n {textScript.PlayerInput}";
            //Inputtext.text += PlayerInput.ToString();
        }
        if (gameMode == "Setup_p2")
        {
            // 最初の指示メッセージ
            InputText.text = $"Player2の秘密の3桁の数字（重複なし）を入力し、Enterで決定してください。\n {textScript.PlayerInput}";
            //Inputtext.text += PlayerInput.ToString();
        }
        else if (gameMode == "PlayerTurn")
        {
            // プレイヤーの推測ターンを開始
            InputText.text = $"--- Player1の推測ターン！ ---\n";
            InputText.text += " Player2の秘密の数字を推測し、Enterを押してください。\n";
            InputText.text += textScript.PlayerInput.ToString();
        }
        else if (gameMode == "OppositPlayerTurn")
        {
            InputText.text = $"--- Player2の推測ターン！ ---\n Player1の秘密の数字を推測し、Enterを押してください。\n{textScript.PlayerInput}";

        }
        if (cpuGuessCount == maxcunt)
        {
            DrawGame();
        }
    ...
    /// <summary>プレイヤーの秘密の数字を設定します。</summary>
    private void HandleOppositPlayerBaseInput()
    {
        int playerInput = textScript.PlayerInput;

        if (InputCheckAndSetBaseDigits(playerInput, cpuBaseNumber))
        {
            // 秘密の数字を設定完了
            gameMode = "PlayerTurn";
            CPUBaseText.text = $"あなたの秘密の数字: ???";
    ...
    private void HandlePlayerGuess()
    {
        ...
        if (InputCheckAndSetBaseDigits(playerInput, tempDigits))
        {
            ...
            {
                // プレイヤーのターンが終わったら相手のターンへ移行
                gameMode = "OppositPlayerTurn";

            }
        }
        textScript.PlayerInput = 0;
    }
    //相手プレイヤーの推測を処理し、判定結果をCPUInputTextに表示
    private void HandleOppositPlayerGuess()
    {
        int playerInput = textScript.PlayerInput;
        // gameManagerScript.PlayerInput = 0; 
        ...
        {
            cpuGuessCount++;
            var result = CheckHitBlow(tempDigits, playerBaseNumber);

            // ★★★ プレイヤー推測履歴の出力 ★★★
            string historyLine = $"\n試行 {cpuGuessCount}: **{playerInput:D3}** -> 判定: **{result.hit}ヒット {result.blow}ブロー**";
            CPUInputText.text += historyLine;
    ...
    private void EndGame(int correctGuess)
    {
        ...
        CPUInputText.text = $"\n\n🎉 **Player2の勝利!!** 秘密の数字 {correctGuess:D3} を当てました！\nPlayer2は**{cpuGuessCount}回目**で正解しました！\n";
        PlayerBaseText.text = $"Player1の秘密の数字: {playerBaseNumber[0]}{playerBaseNumber[1]}{playerBaseNumber[2]}";
        CPUBaseText.text = $"Player2の秘密の数字: {cpuBaseNumber[0]}{cpuBaseNumber[1]}{cpuBaseNumber[2]}";
